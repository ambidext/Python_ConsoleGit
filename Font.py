...
def main():
    LEDBoard = FillWithFont("ABC")
    DisplayLED(LEDBoard)

def DisplayLED(board):
    for i in range(H):
        for j in range(W):
            if (board[i][j] == 0):
                print('.', end='')
...
def fill(result, inputStr):
    for i in range(len(inputStr)):
        fi = fontInfo[ord(inputStr[i])]
        hexStr = "%0.16X" % fi

        for h in range(H):
            sub = hexStr[h * 2:h * 2+2]
            binStr = bin(int(sub,16))[2:]
            binStr = "{:0^8}".format(binStr)

            for b in range(fontSize):
                if binStr[b] == '0':
                    result[h][i * (fontSize+1)+b] = 0
                else:
                    result[h][i * (fontSize+1)+b] = 1

def FillWithFont(inputStr):
